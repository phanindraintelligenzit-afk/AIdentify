...
Captures leads, scores them via AI qualification, stores in SQLite CRM,
and sends Slack alerts for hot leads (score >= 70).

Usage:
    python src/agentkit/templates/lead_qualifier/agent.py --dry-run
    python src/agentkit/templates/lead_qualifier/agent.py --config my_config.yaml
    python src/agentkit/templates/lead_qualifier/agent.py --json
...
def score_budget(budget: int, config: dict) -> int:
    cfg = config.get("scoring", {}).get("budget", {})
    if budget >= cfg.get("tier1_min", 5000):
        return 30
    elif budget >= cfg.get("tier2_min", 1000):
        return 15
    return 5


def score_timeline(months: int, config: dict) -> int:
    cfg = config.get("scoring", {}).get("timeline", {})
    if months <= cfg.get("immediate_months", 1):
        return 25
    elif months <= cfg.get("short_months", 3):
        return 15
    return 5


def score_company_size(size: int, config: dict) -> int:
    cfg = config.get("scoring", {}).get("company_size", {})
    if size >= cfg.get("enterprise_min", 50):
        return 20
    elif size >= cfg.get("mid_min", 10):
        return 12
    return 5


def score_industry(industry: str, config: dict) -> int:
    target = config.get("target_industries", ["ecommerce", "saas", "local_business"])
    if industry in target:
        return 15
    # Partial match for related
...
def score_engagement(source: str, config: dict) -> int:
    cfg = config.get("scoring", {}).get("engagement", {})
    return cfg.get(source, 3)


def score_lead(lead: dict, config: dict) -> dict:
    """Score a lead and return breakdown."""
    breakdown = {
        "budget": score_budget(lead.get("budget", 0), config),
        "timeline": score_timeline(lead.get("timeline_months", 99), config),
        "company_size": score_company_size(lead.get("company_size", 1), config),
        "industry_fit": score_industry(lead.get("industry", "other"), config),
        "engagement": score_engagement(lead.get("source", "cold"), config),
    }
...
def upsert_lead(lead: dict, score: int, dry_run: bool) -> str:
...
def run_qualifier(config: dict, dry_run: bool) -> dict:
    ...
    hot_threshold = config.get("scoring", {}).get("hot_threshold", 70)
    ...
    for lead in leads:
        ...
        lead_id = upsert_lead(lead, total, dry_run)
...
Examples:
  python src/agentkit/templates/lead_qualifier/agent.py --dry-run
  python src/agentkit/templates/lead_qualifier/agent.py --config my_config.yaml
  python src/agentkit/templates/lead_qualifier/agent.py --dry-run --json
""",
    )
    parser.add_argument(
        "--dry-run",
