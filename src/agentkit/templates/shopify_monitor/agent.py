...
Monitors Shopify store for new orders, flags anomalies, sends daily summary.

Usage:
    python src/agentkit/templates/shopify_monitor/agent.py --dry-run
    python src/agentkit/templates/shopify_monitor/agent.py --config my_config.yaml
    python src/agentkit/templates/shopify_monitor/agent.py --json
...
def ensure_tables() -> None:
    ...
    conn.executescript(
        ...
        "total_value REAL DEFAULT 0, currency TEXT DEFAULT 'USD', "
        "shipping_country TEXT, shipping_city TEXT, "
        "line_items_json TEXT DEFAULT '[]', status TEXT DEFAULT 'new', "
        "anomaly_flags TEXT DEFAULT '[]', created_at TEXT, "
...
def check_anomalies(order: dict, config: dict) -> list[str]:
    ...
    max_val = thresholds.get("max_order_value", 5000)
    min_val = thresholds.get("min_order_value", 0.5)
    bulk_qty = thresholds.get("bulk_quantity", 10)
    high_risk = thresholds.get("high_risk_countries", ["NG", "PK", "BD", "VN"])

    total_value = order.get("total_value", 0)

    if total_value > max_val:
        flags.append(f"HIGH_VALUE: ${total_value:.2f} > ${max_val:.2f} threshold")

    if total_value < min_val:
        flags.append(f"SUSPICIOUS_LOW: ${total_value:.2f} < ${min_val:.2f} threshold")
...
def send_email_notification(email_to: str, subject: str, body: str) -> dict:
    ...
    return {"status": "simulated", "message": f"Email sent to {email_to}"}
...
def run_monitor(config: dict, dry_run: bool) -> dict:
    ...
    if output_config.get("save_report", True):
        report_path = PROJECT_ROOT / output_config.get("report_path", "data/shopify_daily_report.md")
    ...
    if anomalies:
        slack_result = send_slack_notification(
            notifications.get("slack_webhook", ""),
...
Examples:
  python src/agentkit/templates/shopify_monitor/agent.py --dry-run
  python src/agentkit/templates/shopify_monitor/agent.py --config my_config.yaml
  python src/agentkit/templates/shopify_monitor/agent.py --dry-run --json
""",
    )
    parser.add_argument(
        "--dry-run",
...
def main() -> None:
    ...
    dry_run = args.dry_run or True  # Always dry-run unless explicitly overridden
